...
public class CreateModel : PageModel
{
    ...
    public class InputModel
    {
        ...
        public string MatchPattern { get; set; } = @"^/api/v1/.*";
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        var route = new GatewayRoute
        {
            ...
            Scopes = selectedScopes.Select(s => new GatewayRouteScope { Scope = s }).ToList()
        };

        var success = await _gatewayService.CreateRouteAsync(route);
        if (success)
        {
            TempData["SuccessMessage"] = $"Pomyślnie utworzono nową regułę routingu API Gateway dla wzorca '{route.MatchPattern}'.";
            return RedirectToPage("Index");
        }

        ModelState.AddModelError(string.Empty, "Wystąpił błąd podczas zapisywania trasy w bazie danych.");
        await LoadAvailableScopesAsync();
        return Page();
    }
